...
def test_electronozm():
    ...
    np.testing.assert_almost_equal(lsy,0.071974056347549428)
    lic=np.trapz(ozm.specic*ozm.outspecene**2*u.eV.to('erg'),ozm.outspecene)
    np.testing.assert_almost_equal(lic,212255584.80106944)
...
def test_electronozm_evolve():
    ...
    lsy=np.trapz(ozm.specsy*ozm.outspecene**2*u.eV.to('erg'),ozm.outspecene)
    np.testing.assert_almost_equal(lsy,24600421942.782776)
    lic=np.trapz(ozm.specic*ozm.outspecene**2*u.eV.to('erg'),ozm.outspecene)
    np.testing.assert_almost_equal(lic,1.0455302369906613e+20)
...
def test_protonozm():
    ...
    np.testing.assert_almost_equal(lpp,3.2800627079738687e+23)
